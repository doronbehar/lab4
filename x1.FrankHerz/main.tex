...
\begin{figure}[H]
    \begin{subfigure}[b]{\textwidth}
    	\centering
    	\resizebox{\textwidth}{!}{\input{heaterCurrentChange_0.pgf}}
    ...
    \begin{subfigure}[b]{\textwidth}
    	\centering
    	\resizebox{\textwidth}{!}{\input{heaterCurrentChange_1.pgf}}
...
הלקוח מתוך
\cite{NIST_ASD}.
זהו המעבר שצפינו שיתרחש שכן ההסתברות להתרחשותו הכי גבוהה מבין כל המעברים, על פי ערך
ה-
...
הפרמטר הראשון אליו נתייחס הוא מתח העצירה, הבחנו כי בממוצע המרחק ביו המקסימות גדל בעת הגדלת מתח העצירה .
עבור ערכים גבוהים מידיי של מתח עצירה רוב האלקטרונים מסוננים ולא מתקבלות מדידות של מקסימות, כפי שאנו רואים במדידה.
אנו יודעים שעבור ערכים נמוכים של מתח ההאטה לא יתבצע סינון כלל, לא הגענו לטווחים כאלו במדידות שלנו.
על כן, קבענו כי טווחי מתח העצירה בהם ניתן לבצע את הניסוי הם -
$(0.5 - 2.5)[v]$.
שכן עבור הערכים בטווח הנ"ל שמדדנו ראינו שהערך הנמדד העומד במבחן רווח בר-סמך.

לאחר מכן בחנו את השפעת הטמפרטורה, וחילקנו את התוצאות לשני חלקים שונים.
עבור המדידות בגרף
\cite{fig:Changing_temp_0-10v},
- בטווח של
$(132.1 - 148.5)^{\circ}C$,
הצלחנו לקבל מקסימות אך המערכת הקריסה את המדידה לפני שהצלחנו למדוד מקסימות נוספות.
...
$169.5 ^{\circ}C$
הצלחנו למדוד יותר מקסימות אך קבענו כי טווח ערכים זה תואם את תנאי הניסוי.
